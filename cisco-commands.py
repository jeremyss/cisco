...
parser = argparse.ArgumentParser(description='Cisco Remote networking device command processor')
...
def _run_command(command, results, lineHost, session):
    ...
    session.expect(prompt + '#', timeout=120)
    ...
    for lineHost in hostList:
        ...
        try:
            session = pexpect.spawn(
                ...
                + lineHost.strip(), timeout=10, maxread=65535)
            session.expect('.*assword.')
            session.sendline(password)
            session.expect('#')
        except:
            print "SSH connection to %s failed, skipping to telnet..." % lineHost

        try:
            session = pexpect.spawn(
                "telnet " + lineHost.strip(), timeout=10, maxread=65535)
            session.expect(r'.*sername|.*ogin')
            session.sendline(username)
            session.expect('.*assword')
            session.sendline(password)
            session.expect('#')
        except:
            print "telnet connection to %s failed, no more options available..." % lineHost
            continue
